...
class SQLiteHandler:
    ...
    def _packet_list_select(cls, include_raw: bool) -> str:
        return cls._PACKET_LIST_SELECT_RAW if include_raw else cls._PACKET_LIST_SELECT_PLAIN
        ...
        try:
            with self._connect() as conn:
                ...
                packets = conn.execute(
                    f"{self._packet_list_select(include_raw)} ORDER BY timestamp DESC LIMIT ?",
                    (limit,),
                ).fetchall()
    ...
    def get_filtered_packets(
        ...
        try:
            with self._connect() as conn:
                ...
                base_query = self._packet_list_select(include_raw)
